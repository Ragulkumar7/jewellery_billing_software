...
export default function ShopifySync() {
  ...
  async function runSync(scope: 'Products' | 'Inventory' | 'Orders' | 'Customers' | 'Everything') {
    ...
    try {
      ...
      await api(`/api/shopify/sync/${scopePath}`, { method: 'POST' });
      await load();
      showToast(`${scope} synchronization completed`);
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === 'customers' && <CustomersView customers={customers} logs={customerLogs} query={query} setQuery={setQuery} syncBadge={syncBadge} onSync={() => runSync('Customers')} onRetry={retry} />}
